...
namespace MonoHexGrid {
  ...
  public abstract class MonoPiece : Node2D {
    ...
    /// </summary>
    public abstract int move_cost(MonoTile src, MonoTile dst, int orientation);

    /// <summary>
    /// are you allowed to move into that Tile as only move even if you don't have enough movement points
    /// </summary>
    public abstract bool at_least_one_tile(MonoTile dst);
    ...
    /// </summary>
    public abstract int volume_of_fire(int category, int distance, MonoTile src, int src_o, MonoTile dst, int dst_o);
  }
}
