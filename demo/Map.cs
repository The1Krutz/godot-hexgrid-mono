...
namespace Demo {
  public class Map : Sprite {
    ...
    MonoHexBoard board;
    Vector2 prev;
    Dictionary hexes; // TODO - need types for these generics
    int hex_rotation;
    Vector2 p0;
    Vector2 p1;
    Array los; // TODO - need types for these generics
    Array move; // TODO - need types for these generics
    Array shoort; // TODO - need types for these generics
    Array influence; // TODO - need types for these generics
    Unit unit;
    bool show_los;
    bool show_move;
    bool show_influence;
    ...
    public void reset() {
      ...
      var Tank = GetNode<Unit>("Tank");
      var Target = GetNode<Unit>("Tank");
      var Hexes = GetNode<Unit>("Tank");

      Tank.Position = board.center_of(p0);
      Target.Position = board.center_of(p1);
      foreach (Hex hex in Hexes.GetChildren()) {
    ...
    public void rotate_map() {
      texture = GD.Load((is_instance_valid(board) and board.v) ? MAPH : MAPV);

      texture = load( if  else )
    	configure()
